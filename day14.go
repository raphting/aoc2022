...
func day14_2(input string) string {
	walls := make(map[int64]map[int64]bool)
	var lowestY int64
	...
	startY := int64(0)
	for {
		newX, newY := getNextSandPosition2(startX, startY, lowestY, walls, restingSand)
...
func getNextSandPosition2(currentX, currentY, lowestY int64, walls, restingSand map[int64]map[int64]bool) (int64, int64) {
	// try direct down
	newX := currentX
	newY := currentY + 1
